...
function fullychat(val){
	$.ajax({
		type : "POST",
		url : basePath+"/group/message/earlierChat",
		data : {
		},
		dataType : "json",
					...
					if(value.from_flag == '1'){
						section += "<div class=\"chat_from\" >" +
										"<div class=\"from_avatar\">" +
											"<img class=\"from_avatar\" src=\""+basePath+"/"+value.avatar+"\" />" +
										"</div>" +
						...
						section += "<div class=\"chat_to\" >" +
										"<div class=\"to_avatar\">" +
											"<img class=\"to_avatar\" src=\""+basePath+"/"+value.avatar+"\" />" +
										"</div>" +
...
if ('WebSocket' in window) {
	websocket = new WebSocket("ws://10.10.2.52:4040/es-chat/webgroupchat");
} else {
...
websocket.onmessage = function(event) {
	var msg  = event.data;
	var to_avatar = msg.substring(0,msg.indexOf("@"));
	var to_account = msg.substring(msg.indexOf("@")+1,msg.indexOf(":"));
...
function sendMsg(){
	...
	var message = from_avatar + "@" + from_account + ":" + msg;
